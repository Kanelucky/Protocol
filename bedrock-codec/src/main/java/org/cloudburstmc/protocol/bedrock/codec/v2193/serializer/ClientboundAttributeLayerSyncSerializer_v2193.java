...
public class ClientboundAttributeLayerSyncSerializer_v2193 extends ClientboundAttributeLayerSyncSerializer_v1001 {
    ...
    public static final ClientboundAttributeLayerSyncSerializer_v2193 INSTANCE =
            new ClientboundAttributeLayerSyncSerializer_v2193();
    ...
    protected EnvironmentAttributeData readEnvironmentAttribute(ByteBuf buf, BedrockCodecHelper helper) {
        ...
        NoiseAlignment na =
                new NoiseAlignment(NoiseAlignment.Type.values()[buf.readUnsignedByte()], VarInts.readUnsignedInt(buf));

        return new EnvironmentAttributeData(
                name, from, attribute, to, currentTicks, totalTicks, easing, localTransitionTicks, noiseTransition, na);
    }
}
